...
import { useCreateCategoryBlogMutation } from '../../slices/categoryBlogApiSlice';
...
const AddCategoryBlog = () => {
  ...
  const [createCategoryBlog, { isLoading: isCreating }] =
  ...
  return (
    <>
      <Link
        to='/admin/categorybloglist'
        className='btn btn-dark text-white mt-2'
      >
        <IoMdArrowBack style={{ margin: '5px' }}></IoMdArrowBack>Quay lại
      </Link>
      <FormContainer>
        <h3 className='text-black text-center distance'>THÊM DANH MỤC BÀI VIẾT</h3>
        {isCreating && <Loader />}
